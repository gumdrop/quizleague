  ...
  val template = """
    ...
    <v-layout column  v-if="fixtures">
      <v-flex v-for="fixs in fixtures" :key="fixs.id">
        <v-card>
          <v-card-title primary-title><h3 class="headline mb-0">{{fixs.date | date('d MMM yyyy')}} - {{async(fixs.parent).name}} {{fixs.description}}</h3><span style="margin-left:1em"></span><ql-questions-link :fixtures="fixs"><span style="font-size:65%;">questions </span><v-icon small color="blue">mdi-open-in-new</v-icon></ql-questions-link></v-card-title>
          <v-card-text>
