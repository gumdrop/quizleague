  ...
  val template = """
    ...
    <v-layout column  v-if="fixtures">
      <v-flex v-for="fixs in fixtures" :key="fixs.id">
        <v-card>
          <v-card-title primary-title><ql-responsive-header>{{fixs.date | date('d MMM yyyy')}} - {{async(fixs.parent).name}} {{fixs.description}}</ql-responsive-header><span style="margin-left:1em"></span><ql-questions-link :fixtures="fixs"><span style="font-size:65%;">questions </span><v-icon small color="blue">mdi-open-in-new</v-icon></ql-questions-link></v-card-title>
          <v-card-text>
